...
def read_and_parse_input():
    ...
    return (tuple(grid_heights), start, end)


def get_moves_at_position(grid, position):
    """
    Given a position within a grid, return all traversable neighbors of the position
    """
    moves = []
...
def dijkstra(grid, start, end):
    ...
    while not queue.empty():
        ...
        position_moves = get_moves_at_position(grid, position)
...
def main():
    grid, start, end = read_and_parse_input()

    path_start_end = dijkstra(grid, start, end)
    # print(path_start_end)
    print(len(path_start_end))
